...
void Renderer::CreateGrassDescriptorSets() {
	...
	grassModelDescriptorSets.resize(scene->GetBlades().size());
	...
	allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
	allocInfo.descriptorPool = descriptorPool;
	allocInfo.descriptorSetCount = static_cast<uint32_t>(grassModelDescriptorSets.size());
	allocInfo.pSetLayouts = layouts;

	// Allocate descriptor sets
	if (vkAllocateDescriptorSets(logicalDevice, &allocInfo, grassModelDescriptorSets.data()) != VK_SUCCESS) {
		throw std::runtime_error("Failed to allocate descriptor set");
	}

	std::vector<VkWriteDescriptorSet> descriptorWrites(grassModelDescriptorSets.size());
	...
	for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
		...
		descriptorWrites[i].dstSet = grassModelDescriptorSets[i];
...
void Renderer::CreateComputeDescriptorSets() {
	...
	for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
		...
		bladesBufferInfo.range = args.bladeNum * sizeof(Blade);
		...
		culledBladesBufferInfo.range = args.bladeNum * sizeof(Blade);
...
void Renderer::RecordComputeCommandBuffer() {
	...
	for (uint32_t j = 0; j < scene->GetBlades().size(); ++j) {
		...
		// Dispatch
		uint32_t groupCountX = (args.bladeNum + WORKGROUP_SIZE - 1) / WORKGROUP_SIZE;
